    ...
    const sendEmailTool = tool(
      async ({ email, userMessage }) => {
        try {
          ...
          const toEmail =
            process.env.ALLOWED_MAILID_FOR_EMAILS ||
            process.env.ALLOWED_MAILID_FOR_MESSAGES ||
            'yashtripathifelix@gmail.com';
          if (!toEmail)
            return 'Could not send email. Admin email not configured.';
      ...
      },
      {
        ...
        description:
          'Send an email to Yash (the portfolio owner). Use this when the user wants to leave a message, contact, or email Yash. You must ask for their email address and the message content if you do not have it.',
        schema: z.object({
          email: z
            .string()
            .email()
            .describe('The email address of the person sending the message'),
          userMessage: z
            .string()
            .describe('The content of the message they want to send'),
        }),
      },
    );
